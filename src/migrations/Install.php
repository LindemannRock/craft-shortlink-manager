...
class Install extends Migration
{
    ...
    public function safeUp(): bool
    {
        ...
        if (!$this->db->tableExists('{{%shortlinkmanager_analytics}}')) {
            ...
            $this->createIndex(null, '{{%shortlinkmanager_analytics}}', ['linkId', 'dateCreated']);
        ...
        if (!$this->db->tableExists('{{%shortlinkmanager_settings}}')) {
            $this->createTable('{{%shortlinkmanager_settings}}', [
                ...
                // URL settings
                'shortlinkBaseUrl' => $this->string(500)->null()->comment('Optional absolute base URL with optional site tokens ({siteHandle}, {siteId}, {siteUid})'),
        ...
        if ($this->db->tableExists('{{%shortlinkmanager_settings}}') && !$this->db->columnExists('{{%shortlinkmanager_settings}}', 'usePrefix')) {
            $this->addColumn(
                ...
                'usePrefix',
                $this->boolean()->notNull()->defaultValue(true)->after('shortlinkBaseUrl')
            );
        }
        ...
        if (!$this->db->tableExists('{{%shortlinkmanager_import_history}}')) {
            ...
            $this->createIndex(null, '{{%shortlinkmanager_import_history}}', 'dateCreated');
            $this->createIndex(null, '{{%shortlinkmanager_import_history}}', 'userId');
            $this->addForeignKey(
                null,
