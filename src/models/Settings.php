...
class Settings extends Model
{
     ...
     * Supports optional tokens: {siteHandle}, {siteId}, {siteUid}
    ...
    protected function defineRules(): array
    {
        return array_merge([
            ...
            [['shortlinkBaseUrl'], 'validateShortlinkBaseUrl'],
    ...
    /**
     * Validate shortlink base URL format (plain URL or tokenized URL).
     */
    public function validateShortlinkBaseUrl(string $attribute, mixed $params, mixed $validator): void
    {
        ...
        if (!preg_match('/^https?:\/\//i', $baseUrl)) {
            $this->addError($attribute, Craft::t('shortlink-manager', 'Shortlink base URL must start with http:// or https://'));
            return;
        }

        if (strpos($baseUrl, '{') !== false && !preg_match('/\{siteHandle\}|\{siteId\}|\{siteUid\}/', $baseUrl)) {
            $this->addError($attribute, Craft::t('shortlink-manager', 'Unsupported token in shortlink base URL. Supported tokens: {siteHandle}, {siteId}, {siteUid}.'));
        }
    }
    ...
    public function buildPublicUrl(string $path, ?int $siteId = null, array $params = []): string
    {
        ...
        if ($baseUrl !== '') {
            $base = $this->expandShortlinkBaseUrl($baseUrl, $siteId);
            if ($base !== '') {
                $url = rtrim($base, '/');
                if ($relativePath !== '') {
                    $url .= '/' . $relativePath;
    ...
    /**
     * Build a public action URL on the configured ShortLink base URL.
     *
    ...
    public function buildPublicActionUrl(string $action, ?int $siteId = null, array $params = []): string
    {
        ...
        $url = rtrim($this->expandShortlinkBaseUrlForActions($baseUrl, $siteId), '/');
    ...
    private function expandShortlinkBaseUrlForActions(string $baseUrl, int $siteId): string
    {
        $expandedBaseUrl = $this->expandShortlinkBaseUrl($baseUrl, $siteId);
     ...
     * @since 5.20.0
     */
    public function getResolvedNotFoundRedirectUrl(): string
    {
    ...
    public function attributeLabels(): array
    {
        return array_merge([
            ...
            'enableQrCodeCache' => Craft::t('shortlink-manager', 'Enable QR Code Cache'),
            'defaultQrErrorCorrection' => Craft::t('shortlink-manager', 'Error Correction Level'),
            'qrLogoVolumeUid' => Craft::t('shortlink-manager', 'Logo Asset Volume'),
            'qrDownloadFilename' => Craft::t('shortlink-manager', 'Download Filename Pattern'),
            'qrPrefix' => Craft::t('shortlink-manager', 'QR Code URL Prefix'),
            'qrTemplate' => Craft::t('shortlink-manager', 'QR Code Template'),
            'passQueryParams' => Craft::t('shortlink-manager', 'Pass Query Params'),
            'directRedirect' => Craft::t('shortlink-manager', 'Direct Redirect'),
            'redirectTemplate' => Craft::t('shortlink-manager', 'Redirect Template'),
            'expiredTemplate' => Craft::t('shortlink-manager', 'Expired Template'),
            ...
            'cacheDeviceDetection' => Craft::t('shortlink-manager', 'Cache Device Detection'),
            'deviceDetectionCacheDuration' => Craft::t('shortlink-manager', 'Device Detection Cache Duration'),
            'enabledIntegrations' => Craft::t('shortlink-manager', 'Integrations'),
            'redirectManagerEvents' => Craft::t('shortlink-manager', 'Automatic Redirect Creation'),
            'seomaticTrackingEvents' => Craft::t('shortlink-manager', 'Tracking Events'),
            'seomaticEventPrefix' => Craft::t('shortlink-manager', 'Event Prefix'),
