...
class SecurityController extends Controller
{
    ...
    {
        $this->stdout("ShortLink Manager - IP Hash Salt Generator\n", Console::FG_CYAN);
        ...
        if (!file_exists($envPath)) {
            ...
            $this->stdout("SHORTLINK_MANAGER_IP_SALT=\"{$salt}\"\n\n", Console::FG_GREEN);
            return ExitCode::OK;
        }

        // Read current .env file
        $envContent = file_get_contents($envPath);
        $saltExists = preg_match('/^SHORTLINK_MANAGER_IP_SALT=/m', $envContent);

        if ($saltExists) {
            $this->stdout("Existing SHORTLINK_MANAGER_IP_SALT found in .env\n\n", Console::FG_YELLOW);
            ...
            $envContent = preg_replace(
                '/^SHORTLINK_MANAGER_IP_SALT=.*$/m',
                'SHORTLINK_MANAGER_IP_SALT="' . $salt . '"',
                $envContent
            );
            $action = "Updated";
        } else {
            ...
            $envContent .= "\n# ShortLink Manager IP Hash Salt (generated " . date('Y-m-d H:i:s') . ")\n";
            $envContent .= 'SHORTLINK_MANAGER_IP_SALT="' . $salt . '"' . "\n";
            $action = "Added";
        }
        ...
        $this->stdout("\n✓ {$action} SHORTLINK_MANAGER_IP_SALT in .env file\n", Console::FG_GREEN);
