...
If your **ShortLink URL Prefix** is `go`, use `/go/*` and `/{siteIdentifier}/go/*` instead. Here `{siteIdentifier}` means each exact site handle, numeric ID, or UID your generated URLs use. If QR scan analytics also need every request to reach Craft, do the same for your **QR Code URL Prefix**. With the default QR prefix this is:
...
For a nested QR prefix such as `s/qr`, use `/s/qr/*` and `/{siteIdentifier}/s/qr/*`. Replace `{siteIdentifier}` with the values generated by your chosen token, for example `/en/s/*` for `{siteHandle}`, `/2/s/*` for `{siteId}`, or the site's UID path for `{siteUid}`.
...
5. **Does a generated multisite path contain a current identifier?** `{siteHandle}`, `{siteId}`, and `{siteUid}` paths match exact current Craft site values. A renamed handle, deleted-site ID, stale UID, random number, or UUID-shaped value correctly returns 404. Regenerate or update any stored/external URL that still carries an old identifier.
...
**Fix:** Use `shortlinkBaseUrl` with `{siteHandle}`, `{siteId}`, or `{siteUid}` instead. Handles are usually the easiest to read:
...
This produces site-specific URLs (`https://short.ly/en/s/abc123`, `https://short.ly/fr/s/abc123`) and registers site-aware redirect and QR routes so the controllers resolve the exact site from the URL path. Numeric ID and UID tokens use the same route behavior.

> [!TIP]
> Rule of thumb: Single-site or headless with one site → `shortlinkBaseUrl`. Multisite → tokenized `shortlinkBaseUrl`; prefer `{siteHandle}` for readable URLs unless IDs or UIDs better suit your deployment.
